...
loadnav = function () {
    ...
    tablist = $("#tab_list_add").Tab({
        items: [
            { id: 'desktop', title: '欢迎首页', closed: false, icon: 'fa fa fa-desktop', url: contentPath + 'jsp/home/adminDefaltDesktop.jsp' },
        ],
        tabScroll: false,
    ...
    $.each(authorizeMenuData, function (i) {
        ...
        if (row.menu.id == 'root') {
            index++;
            _html += '<li class="item">';
            _html += '    <a id=' + row.id + '  href="javascript:void(0);" class="main-nav">';
            _html += '        <div class="main-nav-icon"><i class="fa ' + row.img + '"></i></div>';
            _html += '        <div class="main-nav-text">' + row.name + '</div>';
            _html += '        <span class="arrow"></span>';
            _html += '    </a>';
            _html += getsubnav(row.id);
            _html += '</li>';
            navJson[row.id] = row;
        }
    });
    ...
    $('#nav li a').click(function () {
        ...
        if (data.target == "iframe") {
            tablist.newTab({ moduleIdCookie: true, id: id, title: data.name, closed: true, icon: data.img, url: contentPath + data.url });
        }
    })
    ...
    function getsubnav(moduleId) {
        ...
        $.each(authorizeMenuData, function (i) {
            var row = authorizeMenuData[i];
            if (row.menu.id == moduleId) {
                if (isbelowmenu(row.id) == 0) {
                    _html += '<li><a id=' + row.id + '><i class="fa ' + row.img + '"></i>' + row.name + '</a></li>';
                } else {
                    _html += '<li class="sub" title=' + (row.description == null ? "" : row.description) + '><a id=' + row.id + '><i class="fa ' + row.img + '"></i>' + row.name + '</a>';
                    _html += getchildnav(row.id);
                    _html += '</li>'; //sub
                }
                navJson[row.id] = row;
            }
    ...
    function getchildnav(moduleId) {
        ...
        $.each(authorizeMenuData, function (i) {
            var row = authorizeMenuData[i];
            if (row.menu.id == moduleId) {
                _html += '<li title=' + (row.description == null ? "" : row.description) + '><a id=' + row.id + '><i class="fa ' + row.img + '"></i>' + row.name + '</a></li>';
                navJson[row.id] = row;
            }
        });
        _html += '</ul></div>';
    ...
    function isbelowmenu(moduleId) {
        var count = 0;
        $.each(authorizeMenuData, function (i) {
            if (authorizeMenuData[i].menu.id == moduleId) {
                count++;
                return false;
            }
        });
...
function IndexOut() {
    dialogConfirm("注：您确定要安全退出本次登录吗？", function (r) {
        if (r) {
            ...
            window.setTimeout(function () {
                $.ajax({
                    url: contentPath + "/login/logout.action",
                    type: "post",
                    dataType: "json",
                    success: function (data) {
                        window.location.href = contentPath + "/jsp/login/index.jsp";
                    }
                });
            }, 500);
        }
