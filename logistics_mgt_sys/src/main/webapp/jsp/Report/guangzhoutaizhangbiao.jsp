...
<form id="form1">
    ...
    <script>
        function queryReport(){
            ...
            var xmmc ='${orgId.id}';
            var bdate = $("#start").val().substring(0,10);
            var edate = $("#end").val().substring(0,10);
            var rpg="GZTZB.rpg"; //报表名称
            var getUrl = "reportJsp/showReportGroup.jsp";//报表请求URL地址
            var url = getUrl+"?rpg="+rpg+"&orgid="+xmmc+"&bdate="+bdate+"&edate="+edate;//拼接请求请求连接并传入GET参数
            dialogOpen({
                id: "shuaihuo",
                title: '广州台账表',
                url: url,
