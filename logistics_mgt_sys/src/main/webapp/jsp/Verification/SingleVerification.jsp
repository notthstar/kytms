...
//        $(".profile-content").find("#" + id).show();
    ...
    function GetGrid(){
        ...
        $gridTable.jqGrid({
            url: "/verification/getVerTPList.action",
            datatype: "json",
            ...
            colModel: [
                ...
                {label: '组织机构', name: 'b.name',index:"b.name" ,width: 100, align: 'center',frozen:true},
                {label: '车牌号', name: 'c.vehicleHead',index:"c.vehicleHead" ,width: 100, align: 'center',frozen:true},
                {label: '提派单号',selectDefault:true, name: 'code', index:"a.code", type:"text",width: 150, align: 'center',frozen:true},
                {label: "核销状态", name: "slt.code", width: 70,align: 'center',
                    formatter: function (cellvalue, options, rowObject) {
                        return top.clientdataItem.HXZT['' + cellvalue + '']
                    }},
                {label: '提派日期', name: 'time', index:"a.time", type:"date",width: 80, align: 'center',frozen:true,
                ...
                {label: "核销", name: "a.jsStatus", width: 100,align: 'center',
                    formatter: function (cellvalue, options, rowObject) {
                        return '<span onclick=\"hexiao(\''+rowObject[0]+'\',\'' + rowObject[8]+'\',\''+rowObject[9]+'\',\''+rowObject[10]+'\')\" class=\"label label-info\" style=\"cursor: pointer;\">核销操作</span>';}
                },
                {label: "台账明细", name: "a.jsStatus", width: 100,align: 'center',
                    formatter: function (cellvalue, options, rowObject) {
                        return '<span onclick=\"feiyongmingxi(\''+ rowObject[14] +'\')\" class=\"label label-info\" style=\"cursor: pointer;\">查看费用明细</span>';}
                },
                {label: "提派费", name: "a.jsStatus", width: 70,align: 'center'},
                {label: "已核销金额", name: "a.yhxmoney", width: 80,align: 'center'},
                {label: "未核销金额", name: "slt.code", width: 80,align: 'center'},
                {label: "总税金", name: "slt.code", width: 80,align: 'center'},
                {label: "核销次数", name: "slt.code", width: 70,align: 'center'},
                {label: "台账类型", name: "slt.code", width: 80,align: 'center',
                    formatter: function (cellvalue, options, rowObject) {
