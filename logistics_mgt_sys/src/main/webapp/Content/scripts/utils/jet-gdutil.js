 ...
 */
(function (config) {
    var ba = navigator.userAgent.toLowerCase(), e = window, da = document, fa = da.documentElement;
    ...
    var ja = "microsoft basic render driver;vmware svga 3d;Intel 965GM;Intel B43;Intel G41;Intel G45;Intel G965;Intel GMA 3600;Intel Mobile 4;Intel Mobile 45;Intel Mobile 965".split(";"), ka = "ActiveXObject" in e, la = "devicePixelRatio" in e && 1 < e.devicePixelRatio || ka && "matchMedia" in e && e.matchMedia("(min-resolution:144dpi)") && e.matchMedia("(min-resolution:144dpi)").matches, oa = T("windows nt"), ra = -1 !== ba.search(/windows nt [1-5]\./), sa = -1 !== ba.search(/windows nt 5\.[12]/), ta = ra && !sa;
    T("windows nt 10");
    var ua = T("windows phone"), va = T("macintosh"), wa = T("Mb2345Browser"), xa = T("ipad;") || T("ipad "), ya = xa && la, za = T("ipod touch;"), Aa = T("iphone;") || T("iphone "), Ba = Aa || xa || za, Ca = Ba && -1 !== ba.search(/ os [456]_/);
    Ba && ba.search(/ os [4-8]_/);
    var Ea = Ba && -1 !== ba.search(/ os [78]_/);
    Ba && T("os 8_");
    var Fa = Ba && T("os 10_"), Ga = T("android"), Ha = -1 !== ba.search(/android [123]/), Ka = T("android 4");
    ...
    var La = Ga ? "android" : Ba ? "ios" : oa ? "windows" : va ? "mac" : "other", Ma = ka && !e.XMLHttpRequest, Na = ka && !da.querySelector, Oa = ka && !da.addEventListener, Pa = ka && T("ie 9"), Qa = ka && T("msie 10"), Ra = ka && T("rv:11"), Sa = T("edge"), Ta = T("qtweb"), Ua = T("ucbrowser"), Va = T("alipay") || Ga && Ua, Wa = T("miuibrowser"), Xa = T("micromessenger"), Ya = T("mqqbrowser"), Za = T("baidubrowser"), chrome = (T("chrome") || T("crios")) && !Xa && !Za && !Ya && !Sa && !Wa, ab = chrome && T("chromium"), bb = chrome && !ab && 30 < parseInt(ba.split("chrome/")[1]), cb = T("firefox"), db = cb &&
    ...
    }(), ob = la && (!ib || !!nb && 1 <= nb), pb = ka && "transition" in fa.style, qb = !!da.createElementNS && !!da.createElementNS("http://www.w3.org/2000/svg", "svg").createSVGRect, rb = da.createElement("canvas"), sb = !(!rb || !rb.getContext), vb = window.URL || window.webkitURL, wb = !ka && !(Ua && Ga) && window.Worker && vb && vb.createObjectURL && window.Blob, xb = "", ia = "", yb = 0, zb = {
        alpha: !0, antialias: !0,
        depth: !1, failIfMajorPerformanceCaveat: !0, preserveDrawingBuffer: !0, stencil: !1
    }, Ab = function () {
        ...
        return !1
    }(), Bb = Ab && !ib && (bb || db || fb) && ("mac" == La || "windows" == La), Cb = !sb || Ta || ua || ib && cb || Pa || Ca || ya || za ||
    ...
    try {
        if ("undefined" === typeof e.localStorage) Qb = !1; else {
    ...
    config.j = {
        ...
        dM: kb,
        eM: lb,
        aX: 57 <= parseInt(ba.split("chrome/")[1]),
        m3: Fb,
        e6: pb,
        n3: Gb,
        y5: Hb,
    ...
    config.Ed = "main";
    config.j.ud && (Ub += ",touch", config.Ed += "t");
    config.j.U || (Ub += ",mouse", config.Ed += "m");
    config.Ed += "c";
    config.j.Je && (config.Ed += "v", Ub += ",vectorlayer,overlay", config.j.Yd ? (config.Ed += "w", Ub += ",wgl") : (config.Ed += "cg", Ub += ",cmng,cgl"));
    if (config[7]) {
        ...
        Ub += "," + Vb.join(",");
        config.Ed += config[7].replace(",", "").replace(eval("/AMap./gi"), "")
    }
    config.Nn = Tb;
    Ub += ",sync";
    config.wN = Ub.split(",");
    window.AMap = window.AMap || {};
    window.AMap.Pg = "1.3.28.4";
    var Zb = window.AMap.PC = {}, mc = config[2].split(",")[0], nc = mc + "/theme/v" + config[4] + "/style1.3.28.4.css", oc = document.head || document.getElementsByTagName("head")[0];
    if (oc) {
    ...
    function rc() {
        for (var a = mc + "/maps/main?v=" + config[4] + "&key=" + config[0] + "&m=" + config.wN.join(",") + "&vrs=1.3.28.4", b = document.getElementsByTagName("script"), c, d = 0; d < b.length; d += 1)if (0 === b[d].src.indexOf(mc.split(":")[1] + "/maps?")) {
            c = b[d];
            break
        }
    ...
    try {
        if (window.localStorage) {
            var tc = window.localStorage["_AMap_" + config.Ed], uc = !1;
            tc ? (tc = JSON.parse(tc), tc.version === window.AMap.Pg ? (eval(tc.script), Zb.loaded = !0) : uc = !0) : uc = !0;
    ...
    Zb.loaded || (rc(), config.wN = void 0);
})(["鎮ㄧ敵璇风殑key鍊�", [120.975302, 38.719833, 123.518921, 40.20519, 121.618622, 38.91459], "https://webapi.amap.com", 1, "1.3", null, "210200", "", true, true, true, true, "1496841095-1"])