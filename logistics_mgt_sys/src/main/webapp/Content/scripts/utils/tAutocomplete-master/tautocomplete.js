(function ($) {
    "use strict";
   //SF 替换BOX参数
    $.fn.tautocomplete = function (options, callback,SF) {
        ...
        function select() {
            ...
            //修改返回模型
            el.ddTextbox.data("data", {
                id :selected.find('td').eq(0).text(), //城市ID
                name :selected.find('td').eq(1).text(),
                zone :selected.find('td').eq(2).text(),
                contactperson :selected.find('td').eq(3).text(),
                iphone :selected.find('td').eq(4).text(),
                address :selected.find('td').eq(5).text(),
                ltl :selected.find('td').eq(6).text(),
            });
