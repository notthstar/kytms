 ...
 * @constructor 臧英明
 */
$.fn.ValidformResule=function(data){
    var Validatemsg = "";
    var Validateflag = true;
    for(var p in data){
        var s = data[p];
        var  key= $('#'+p);
        ValidationMessage(key, s)
    }

    // $(this).find('input,select,textarea,.ui-select').each(function () {
...
$.fn.Validform = function () {
    ...
    $(this).find("[isvalid=yes]").each(function () {
        ...
        if (checkexpession != undefined) {
            ...
            switch (checkexpession) {
                ...
                //陈小龙添加（不能有中文和特殊字符，但是可以有-和_）
                case "ChineseNotStr":
                {
                    if (!isNotChinese(value, $(this).attr("eqvalue"))) {
                        Validatemsg = errormsg + "不能为空,不能包含中文和特殊字符！\n";
                        Validateflag = false;
                        ValidationMessage($(this), Validatemsg); return false;
                    }
                    break;
                ...
                ////陈小龙添加（不能有中文和特殊字符，但是可以有-和_）
                case "ChineseNotStrOrNull":
                {
                    if (!isNotChineseOrNull(value, $(this).attr("eqvalue"))) {
                        Validatemsg = errormsg + "不能包含中文和特殊字符！\n";
                        Validateflag = false;
                        ValidationMessage($(this), Validatemsg); return false;
                    }
                    break;
                ...
                {
                    if (!isCarNum(value)) {
                        Validatemsg = errormsg + "请输入正确车牌格式！\n";
                        Validateflag = false;
                        ValidationMessage($(this), Validatemsg); return false;
                    }
                    break;
                }
                case "IsPositiveOrNull":
                {
                    if (!isPositiveOrNull(value)) {
                        Validatemsg = errormsg + "必须为正数和0！\n";
                        Validateflag = false;
                        ValidationMessage($(this), Validatemsg); return false;
                    }
                    break;
                ...
                case "IsInterOrNull":
                {
                    if (!isInterOrNull(value)) {
                        Validatemsg = errormsg + "必须为正整数和0！\n";
                        Validateflag = false;
                        ValidationMessage($(this), Validatemsg); return false;
                    }
                    break;
                ...
                case "JQyz":
                {
                    if(!jqyz(value, $(this).attr("eqvalue"))){
                        Validatemsg = errormsg + "至少8-16个字符，至少1个大写字母，1个小写字母和1个数字，其他可以是任意字符！\n";
                        Validateflag = false;
                        ValidationMessage($(this), Validatemsg); return false;
                    }
                    break;
    ...
    function isCarNum(obj) {
        reg = /[京津冀晋蒙辽吉黑沪苏浙皖闽赣鲁豫鄂湘粤桂琼川贵云渝藏陕甘青宁新使]{1}[A-Z]{1}[0-9a-zA-Z]{5}$/u;
        if (!reg.test(obj)) {
            return false;
        }else{
            return true;
...
function ValidationMessage(obj, Validatemsg) {
    try {
        removeMessage(obj);
        obj.focus();
        var $poptip_error = $('<div class="poptip"><span class="poptip-arrow poptip-arrow-top"><em>哈哈哈哈</em></span>' + Validatemsg + '</div>').css("left", obj.offset().left + 'px').css("top", obj.offset().top + obj.parent().height() + 5 + 'px')
